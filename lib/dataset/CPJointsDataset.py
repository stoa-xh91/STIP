...
class CPPartsDataset(Dataset):
    ...
    def generate_sem_target(self, center, joints, joints_vis, heatmaps):
        kpt_polar_coords = np.zeros((self.num_joints, 3))
        feat_stride = self.image_size[0] / self.heatmap_size[0]
        if np.sum(joints_vis > 0) == 0:
            return kpt_polar_coords
        hm_height, hm_width = heatmaps.shape[1:]
        kpt_loc = np.copy(joints / feat_stride + 0.5)[joints_vis > 0]
        kpt_loc = np.floor(kpt_loc).astype(np.int32)
        kpt_loc = np.maximum(kpt_loc, 0)
        kpt_loc[:, 0] = np.minimum(kpt_loc[:, 0], hm_width - 1)
        kpt_loc[:, 1] = np.minimum(kpt_loc[:, 1], hm_height - 1)
        center[1] = hm_height - center[1] - 1
        kpt_loc[:, 1] = hm_height - kpt_loc[:, 1] - 1
        vec_x = kpt_loc[:, 0] - center[0] + 1
        vec_y = kpt_loc[:, 1] - center[1] + 1
        length = np.sqrt(vec_x ** 2 + vec_y ** 2) + 1
        theta = np.arccos(vec_x / length) * 360. / 2 / np.pi
        cross_products = vec_y
        theta[cross_products < 0] = 360 - theta[cross_products < 0]
        theta = theta / 360.
        kpt_polar_coords[joints_vis > 0, 0] = np.abs(vec_x / hm_width)
        kpt_polar_coords[joints_vis > 0, 1] = np.abs(vec_y / hm_height)
        kpt_polar_coords[joints_vis > 0, 2] = theta
        return kpt_polar_coords
